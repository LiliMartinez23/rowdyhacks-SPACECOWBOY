...
let ufo2 = Matter.Bodies.trapezoid( 875, 220, ufoBodyW, ufoBodyH, slope, {
    isStatic: true,
    chamfer: { radius: 6 },
    label: 'ufoPlatform',
    render: {
...
let aliens2 = Matter.Composites.stack( 805, 70, 4, 4, 0, 0, ( x, y ) => createAlien( x, y ) );
...
const ub2 = ufo2.bounds;
const ufo2Width = ub2.max.x - ub2.min.x;
// Sensor
const sensor2 = Matter.Bodies.rectangle(
    (ub2.max.x + ub2.min.x) / 2,
    ub2.min.y - 6,
    ufo2Width,
    12,
    {
...
const sensors = new Set([ sensor, sensor2 ]);

// Tracker
const aliensOn = new Set();
const starsOn = new Set();
const alienIds = new Set([
    ...aliens.bodies.map( b => b.id),
    ...aliens2.bodies.map( b => b.id)
]);
...
Matter.Events.on( engine, 'afterUpdate', function seedOnce() {
    ...
    const stacks = [ aliens, aliens2 ];
    const platformSensors = [ sensor, sensor2 ];
...
Matter.Events.on( engine, 'beforeUpdate', function() {
    ...
    Matter.Body.translate( ufo2, { x: -dx, y: 0 } );
    Matter.Body.translate( sensor2, { x: -dx, y: 0 } );
    Matter.Composite.translate( aliens2, { x: -dx, y: 0 } );
});
Matter.Events.on( engine, 'afterUpdate', function mirrorOnce() {
    Matter.Events.off( engine, 'afterUpdate', mirrorOnce );

    const { minX, maxX } = horizontalMotion;
    const mirrorX = minX + maxX - ufo.position.x;
    const dx2 = mirrorX - ufo2.position.x;

    if ( dx2 !== 0 ) {
        Matter.Body.translate( ufo2, { x: dx2, y: 0 } );
        Matter.Body.translate( sensor2, { x: dx2, y: 0 } );
        Matter.Composite.translate( aliens2, { x: dx2, y: 0 } );
    }
});
